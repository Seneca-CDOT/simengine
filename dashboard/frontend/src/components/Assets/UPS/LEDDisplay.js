...
const LEDDisplay = ({ battery, x, y, status, upsMonitorImg }) => {
  ...
  return (
    <Group x={x} y={y}>
      ...
      <Group y={50} x={18}>
        <Text text={`Output ${status?'ON':'OFF'}`} fontFamily={fontFamily} fontSize={fontSize} fill={fill}/>
        <Text y={30} text={`Batt ${Math.floor(battery/10)}%`} fontFamily={fontFamily} fontSize={fontSize} fill={fill}/>
