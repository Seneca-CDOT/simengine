...
from enginecore.state.state_managers import (
    BMCServerStateManager,
    StateManager,
    StaticDeviceStateManager,
)
...
class ServerLoadTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        sm.create_server(
            4, server_attr, server_variation=sm.ServerVariations.ServerWithBMC
        )
        sm.create_static(
            5, {"power_consumption": 240, "power_source": 120, "name": "test"}
        )
    ...
    def check_redis_values(self, expected_kv):
        for key, value in expected_kv.items():
            ...
            self.assertAlmostEqual(
                float(value),
                float(r_value),
                msg="asset [{}] = {}, expected({})".format(key, r_value, value),
            )
    ...
    def test_server_power(self):
        ...
        try:
            ...
            thread = Thread(
                target=wait_redis_update,
                args=(ServerLoadTest.redis_store, "load-upd", server_down, 7),
            )
            thread.start()
            ...
            thread = Thread(
                target=wait_redis_update,
                args=(ServerLoadTest.redis_store, "load-upd", server_up, 7),
            )
            thread.start()
            server.power_up()
            thread.join()
    ...
    def test_static(self):
        static = StaticDeviceStateManager(
            {"key": 5, "name": "test", "powerConsumption": 240, "powerSource": 120},
            notify=True,
        )
        static_up = {
            "1-outlet": 4,
        ...
        thread = Thread(
            target=wait_redis_update,
            args=(ServerLoadTest.redis_store, "load-upd", static_down, 4),
        )
        thread.start()
        ...
        thread = Thread(
            target=wait_redis_update,
            args=(ServerLoadTest.redis_store, "load-upd", static_up, 4),
        )
        thread.start()
    ...
    def test_server_psu(self):
        ...
        try:
            ...
            thread = Thread(
                target=wait_redis_update,
                args=(ServerLoadTest.redis_store, "load-upd", only_psu1_down, 6),
            )
            thread.start()
            ...
            thread = Thread(
                target=wait_redis_update,
                args=(ServerLoadTest.redis_store, "load-upd", both_up, 6),
            )
            thread.start()
            ...
            thread = Thread(
                target=wait_redis_update,
                args=(ServerLoadTest.redis_store, "load-upd", only_psu2_down, 6),
            )
            thread.start()
            ...
            t1 = Thread(
                target=wait_redis_update,
                args=(ServerLoadTest.redis_store, "load-upd", only_psu1_down, 5),
            )
            t2 = Thread(
                target=wait_redis_update,
                args=(ServerLoadTest.redis_store, "load-upd", both_down, 4),
            )
            ...
            t1 = Thread(
                target=wait_redis_update,
                args=(ServerLoadTest.redis_store, "load-upd", only_psu2_down, 5),
            )
            t2 = Thread(
                target=wait_redis_update,
                args=(ServerLoadTest.redis_store, "load-upd", both_up, 4),
            )
