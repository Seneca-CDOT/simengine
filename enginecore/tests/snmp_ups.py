...
class UpsSnmpTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        sm.create_ups(3, {"power_consumption": 240, "power_source": 120, "port": 1024})

        sm.create_static(
            4, {"power_consumption": 240, "power_source": 120, "name": "test_1"}
        )

        sm.create_static(
            5, {"power_consumption": 240, "power_source": 120, "name": "test_2"}
        )

        sm.create_static(
            6, {"power_consumption": 240, "power_source": 120, "name": "test_3"}
        )
    ...
    def get_oid_value(self, oid):
        return subprocess.check_output(
            "snmpget -c public -Oqv -v 1 127.0.0.1:1024 {}".format(oid), shell=True
        )
    ...
    def test_ups_oids(self):
        ...
        )  # UPS Status: On-Battery, Powered etc.
        input_line_fail_cause_oid = (
            "1.3.6.1.4.1.318.1.1.1.3.2.5.0"
        )  # Why switched to the Battery
        hp_output_load_oid = (
            "1.3.6.1.4.1.318.1.1.1.4.3.3.0"
        )  # % from total capacity (mult by 10)
        ...
        thread = Thread(
            ...
            args=(UpsSnmpTest.redis_store, "load-upd", expected_state, 2),
        )
        thread.start()
        sm_out_1.shut_down()
        ...
        self.assertEqual(
            drain_output_status.decode().rstrip(), "onBattery"
        )  # 3 --> onBattery
        self.assertEqual(
            drain_input_line_fail_cause.decode().rstrip(), "deepMomentarySag"
        )
        ...
        thread = Thread(
            target=wait_redis_update,
            args=(UpsSnmpTest.redis_store, "load-upd", expected_state, 3),
        )
        thread.start()
        sm_out_31.shut_down()
        ...
        self.assertNotEqual(
            new_runtime_left, runtime_left
        )  # child is powered down -> battery is being drained
        ...
        self.assertEqual(
            charge_output_status.decode().rstrip(), "onLine"
        )  # back online!
