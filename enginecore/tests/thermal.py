...
class ThermalTest(unittest.TestCase):
    ...
    def _test_outage(self):
        ...
        mains_t = Thread(
            ...
            args=(
                ThermalTest.redis_store,
                RedisChannels.mains_update_channel,
                {"mains-source": 0},
                1,
            ),
        )
    ...
    def test_ambient_dynamic_changes(self):
        ...
        ambient_t = Thread(
            target=wait_redis_channel_upd,
            args=(ThermalTest.redis_store, RedisChannels.ambient_update_channel),
        )
        mains_t = Thread(
            target=wait_redis_channel_upd,
            args=(ThermalTest.redis_store, RedisChannels.mains_update_channel),
        )
